...
<body>
  <div class="app">
    <aside class="sidebar">
      ...
      <button id="openNewChat" class="iconBtn newChatBtn" type="button">+ Agent</button>
      ...
      <div id="newChatOverlay" class="settingsOverlay hidden" aria-hidden="true">
        <div class="settingsPanel" role="dialog" aria-modal="true" aria-labelledby="newChatTitle">
          <div class="settingsHead">
             <h2 id="newChatTitle">Новый агент</h2>
          ...
          <form id="newForm" class="newForm" autocomplete="off">
            ...
            <div class="formField">
              ...
              <div class="modeHint">Агент всегда работает с файлами выбранного проекта.</div>
            </div>

            <label class="formField">
              <span>Название агента (опционально)</span>
            ...
            <div class="formActions">
              <button type="submit" class="iconBtn primaryBtn">Создать агента</button>
            </div>
  ...
  <script>
    ...
    let appState = { conversations: [], activeConversationId: null, workspaceRoot: "", agentMode: false };
    ...
    (async () => {
      ...
      renderProviderPicker();
      renderModePickerForProvider();
      await loadState().catch((error) => setStatus(error.message, true));
    })();
    ...
    document.getElementById("composer").addEventListener("submit", async (event) => {
      ...
      if (isActiveConversationRunning()) {
        setStatus("Агент уже выполняет задачу. Дождись завершения текущей задачи.");
        ensurePolling();
        return;
      }
      ...
      try {
        ...
        // Если сервер вернул running:true — агент работает в фоне. Polling сам подхватит результат.
        if (data.running) {
          activeConversation = data.conversation;
          await loadState(activeConversation.id);
          renderConversation(activeConversation);
          setStatus(data.notice || "Задача выполняется в фоне");
          ensurePolling();
    ...
    async function loadState(nextActiveId = null) {
      ...
      if (appState.agentMode && !nextActiveId && !hasActiveWorkspaceAgent()) {
    ...
    function hasActiveWorkspaceAgent() {
      if (!appState.activeConversationId) return false;
      const active = (appState.conversations || []).find((item) => item.id === appState.activeConversationId);
      if (!active || active.agentMode !== true) return false;
      return normalizeFsPath(active.workspace) === normalizeFsPath(appState.workspaceRoot);
    }

    async function ensureWorkspaceAgent() {
      const provider = availableProviders.includes(newChatSelectedProvider)
        ? newChatSelectedProvider
        : availableProviders[0];
      if (!provider) return;
      const data = await api("/api/agent-session", {
        method: "POST",
        body: {
          provider,
          model: defaultModelForProviderMode(provider, (PROVIDER_INFO[provider] || {}).defaultMode),
        },
      });
      activeConversation = data.conversation;
    ...
    // Polling для отслеживания фоновых агент-задач. Один setInterval на всю сессию,
    ...
    async function pollTick() {
      ...
      // (могут добавляться tool-логи во время работы агента).
    ...
    // Patch на сервере: обновить модель активного агента.
    ...
    function isActiveConversationRunning() {
      return Boolean(
        activeConversation &&
        Array.isArray(appState.runningTaskIds) &&
        appState.runningTaskIds.includes(activeConversation.id)
      );
    }
    ...
    // ---- Settings modal (разрешённые команды агента) ----
